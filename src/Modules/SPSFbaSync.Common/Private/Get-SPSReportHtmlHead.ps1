function Get-SPSReportHtmlHead {
    <#
        ...
        server) with the embedded SPS* report stylesheet, followed by the opening
        <body> tag. The stylesheet includes the status "pill" styles used to render the
        report table (ok / warn / neutral).
...
.pill-warn{background:#fff6e0;color:var(--warn);border:1px solid #f0e0b0}
