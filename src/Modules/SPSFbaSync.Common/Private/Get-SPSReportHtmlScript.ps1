function Get-SPSReportHtmlScript {
    <#
        .SYNOPSIS
        Returns the vanilla-JavaScript block that makes the report table interactive.

        .DESCRIPTION
        Operates on the static <table id="sps-report"> rendered server-side by
        Export-SPSFbaSyncReport (so the status "pills" are real HTML). It wires:

        - a search box (id="report-search") that shows/hides rows by matching text in
          any cell (case-insensitive), and
        - clickable column headers that sort the visible rows. Sorting uses each cell's
          textContent, which for status columns is the pill label.

        The row count indicator (id="report-info") is updated on every filter. No
    ...
    $js = @'
(function(){
  var table = document.getElementById('sps-report');
  if (!table) { return; }
