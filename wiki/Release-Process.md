...
This page documents how to ship a new version of SPSFbaSync. The process is centered on a
single source of truth — the `ModuleVersion` field of `SPSFbaSync.Common.psd1` — and a `v*`
git tag that triggers the GitHub release workflow.

## Versioning policy

SPSFbaSync follows [Semantic Versioning 2.0](https://semver.org/spec/v2.0.0.html).
...
Edit **one** value in `src/Modules/SPSFbaSync.Common/SPSFbaSync.Common.psd1`:

```powershell
ModuleVersion = '2.0.0'   # was '1.1.0'
```

This single change propagates automatically to the script banner
(`$SPSFbaSyncVersion` is read from `(Get-Module SPSFbaSync.Common).Version`).
...
```powershell
Import-Module .\src\Modules\SPSFbaSync.Common\SPSFbaSync.Common.psd1 -Force
(Get-Module SPSFbaSync.Common).Version    # should match the bumped version
...
1. Packages the **contents** of `src/` into `SPSFbaSync-v2.0.0.zip` (the archive extracts
   straight to `SPSFbaSync.ps1` and `Modules\`, with no `src/` wrapper).
2. Publishes a GitHub Release using `RELEASE-NOTES.md` as the body.
3. Attaches the ZIP and `LICENSE` to the release.

### 7. Verify

- **Releases**: <https://github.com/luigilink/SPSFbaSync/releases> — the new release is listed with the expected body and ZIP.
- **Actions**: <https://github.com/luigilink/SPSFbaSync/actions> — `release.yml` and `pester.yml` ran green.
- **Wiki**: <https://github.com/luigilink/SPSFbaSync/wiki> — `wiki.yml` synced any `wiki/` changes pushed in the same release.
